library(dplyr)
library(tidyr)
library(lubridate)  # For easier date/time manipulation
...
na_count_final <- sum(is.na(minuteIntensities))
if (na_count_final > 0) {
...
# Export the data
write.csv(minuteIntensities, file = "hourlyIntensities.csv", row.names = FALSE)